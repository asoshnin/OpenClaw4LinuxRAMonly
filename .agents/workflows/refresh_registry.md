...
1. Find the path to the workspace's `factory.db` (default: `~/.openclaw/workspace/factory.db`).
...
**Note:** If you registered or updated an agent via `register-agent` (Sprint 10+), always run `refresh-registry` immediately after to keep REGISTRY.md in sync:

```bash
python3 openclaw_skills/librarian/librarian_ctl.py register-agent \
  --db-path "$HOME/.openclaw/workspace/factory.db" \
